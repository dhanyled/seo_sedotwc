...
* [ ] **Paritas Database vs Markdown (Post-Sync Audit):** Memastikan nilai `_seopress_titles_title`, `_seopress_titles_desc`, dan `_seopress_analysis_target_kw` di database WordPress identik 100% dengan frontmatter markdown lokal (mencegah desinkronisasi pasca-terbit).
* [ ] **Zero Static Phone Number Policy:** Seluruh nomor telepon di dalam badan halaman/artikel wajib dibungkus tautan klik langsung (`wa.me` berkelas `.wa-btn-static` dengan pola Two-Layer CTA), dilarang teks nomor telepon statis tanpa aksi.
